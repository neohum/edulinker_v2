...
export default function TargetTreeModal({ isOpen, onClose, allUsers, currentTargets, onApply, onlyTeachers }: TargetTreeModalProps) {
  ...
  const students = onlyTeachers ? [] : allUsers.filter(u => u.role === 'student')
  const parents = onlyTeachers ? [] : allUsers.filter(u => u.role === 'parent')
  ...
  const handleConfirmTarget = () => {
    ...
    if (compacted.length === 0) compacted = onlyTeachers ? ['TEACHER'] : ['ALL']

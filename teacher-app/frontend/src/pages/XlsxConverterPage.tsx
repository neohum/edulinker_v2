...
export default function XlsxConverterPage() {
  ...
  const processFile = (selectedFile: File) => {
    ...
    if (name.endsWith('.xlsx') || name.endsWith('.xls')) {
      ...
      toast.error('엑셀 파일(.xlsx, .xls)만 선택 가능합니다.')
    }
  }
  ...
  const handleConvert = async () => {
    ...
    try {
      const wailsApp = (window as any).go?.main?.App
      if (wailsApp?.ConvertExcelToPdf) {
        const base64Data = await fileToBase64(file)
        const res = await wailsApp.ConvertExcelToPdf(file.name, base64Data)
    ...
    <div style={{ padding: 24, maxWidth: 800 }}>
      <div style={{ marginBottom: 32 }}>
        <h3 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>
          <i className="fi fi-rr-file-excel" style={{ marginRight: 12, color: '#16a34a' }} />
          Excel to PDF 변환
        </h3>
        <p style={{ color: 'var(--text-secondary)' }}>
          엑셀 파일을 PDF 문서로 변환합니다. (MS Excel 설치 필요)
        </p>
      </div>

      {officeStatus && !officeStatus.excel && (
        <div style={{ marginBottom: 20, padding: 16, background: '#fef2f2', border: '1px solid #fee2e2', borderRadius: 12, color: '#991b1b', fontSize: 14 }}>
          ...
          PC에 Microsoft Excel이 설치되어 있지 않거나 실행할 수 없습니다.
        </div>
      )}
      ...
      >
        {!file ? (
          <div>
            ...
            <p style={{ fontWeight: 700, fontSize: 18 }}>엑셀 파일을 클릭하여 선택</p>
            <input ref={fileInputRef} type="file" accept=".xlsx,.xls" onChange={handleFileChange} style={{ display: 'none' }} />
          </div>
        ) : (
          <div onClick={e => e.stopPropagation()}>
            <div style={{ fontSize: 48, color: '#16a34a', marginBottom: 16 }}><i className="fi fi-rr-file" /></div>
            <p style={{ fontWeight: 600, fontSize: 18 }}>{file.name}</p>
            <p style={{ color: '#64748b', marginBottom: 24 }}>{formatSize(file.size)}</p>
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
              <button onClick={handleConvert} disabled={converting || !officeStatus?.excel} style={{ background: '#16a34a', color: 'white', padding: '12px 24px', borderRadius: 12, border: 'none', fontWeight: 600, cursor: 'pointer' }}>
